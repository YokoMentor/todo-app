...
export function fetchAtciveItems(): TodoList[] {
    return fetchAll(todoDB, "select id, item, status, created from TODO_ITEM where status = 'Active'", []) as TodoList[];
}

export function clearCompleted(status: string) {
  runQuery(todoDB, "delete from TODO_ITEM where status = 'Completed'", [status]);
} 
